...
`OpenCodeTrueIdleDetector` 位于 `src/opencode-true-idle-detector.js`，是一个可复用的空闲检测器，封装了 ExecutionTracker 状态机 + 200ms 去抖逻辑。
...
```js
new OpenCodeTrueIdleDetector({ log, onIdle })
```
...
prompt() 返回 ── AI 回复完成 → helloLocked = false
        │
        ▼
下一次 TRUE_IDLE（onIdle 回调）─→ 发送下一个 hello（循环）
```

`client.session.prompt()` 是阻塞式调用，天然保证"回复处理完成"后才解锁。用户输入时也会重置 `helloLocked` 和 `helloCount`。
